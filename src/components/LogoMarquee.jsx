...
const logos = [
  { name: 'MANGATA', cls: 'font-serif tracking-[0.35em]' },
  { name: 'OYSTER', cls: 'font-display tracking-[0.2em]' },
  { name: 'NOMAD', cls: 'font-extrabold tracking-[0.25em]' },
  { name: 'BISTRO 21', cls: 'font-serif italic tracking-wider' },
  { name: 'CASA MIA', cls: 'font-serif tracking-[0.25em]' },
  { name: 'KOI', cls: 'font-display tracking-[0.3em]' },
];
  ...
  return (
    <section className="border-y border-ink/15 py-5">
      <div className="mx-auto flex max-w-7xl items-center gap-8 px-4 md:px-8">
        <p className="hidden w-44 shrink-0 text-[10px] font-bold uppercase leading-snug tracking-widest text-ink/70 md:flex md:items-center md:gap-3">
          ...
          <div className="flex w-max items-center gap-16 pr-16 animate-marquee hover:[animation-play-state:paused]">
            {[...logos, ...logos].map((l, i) => (
              <span key={i} className={`flex items-center gap-16 whitespace-nowrap text-sm text-ink/80 ${l.cls}`}>
                {l.name}
                {i % 6 === 5 && <AsteriskIcon className="h-5 w-5 text-ink/50" />}
              </span>
            ))}
          </div>
        </div>
